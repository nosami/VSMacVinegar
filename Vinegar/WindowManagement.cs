...
namespace Vinegar
{
    ...
    internal static class WindowManagement
    {
        ...
        public static int GetNotebookIndex(ITextView textView)
        {
            var notebooks = GetNotebooks();
            for (int i = 0; i < notebooks.Length;i++)
            {
                var index = notebooks[i].FileNames.IndexOf(GetName(textView.TextBuffer));
                if (index != -1)
                {
                    return i;
                }
        ...
        public static string GetName(ITextBuffer textBuffer)
        {
